...
/**/
if ($elementId && $_REQUEST["type"] == 'sale'){
	CModule::IncludeModule('sale');
	$arSelect = Array("ID", "NAME", "IBLOCK_ID");
	$arFilter = Array("IBLOCK_ID"=>\Shantilab\Data::SALING_PROP_IBLOCK_ID, "ACTIVE_DATE"=>"Y", "ACTIVE"=>"Y","ID" =>$elementId);
	$res = CIBlockElement::GetList(Array(), $arFilter, false, Array("nPageSize"=>1), $arSelect);
	if($ob = $res->GetNextElement(false,false))
	{
		$arFields = $ob->GetFields();
		$arFields["PROPERTIES"] = $ob->GetProperties();
		$arResult["SELLING_PROP_ELEMENT"] = $arFields;
	}
}
/**/
...
if ($bAllowAccess)
{
	...
	if (check_bitrix_sessid() && (!empty($_REQUEST["iblock_submit"]) || !empty($_REQUEST["iblock_apply"])))
	{
		...
		//custom check fields
		if (!$arUpdatePropertyValues[27] && !$arUpdatePropertyValues[28]){
			$arResult["ERRORS"][] = 'Необходимо заполнить поле "Email" или "Телефон"';
		}
		//телефон
		if (!preg_match("/^((8|\+7)[\- ]?)?(\(?\d{3}\)?[\- ]?)?[\d\- ]{7,10}$/u",$arUpdatePropertyValues[28])){
			$arResult["ERRORS"][] = 'Неверно введён номер телефона';
		}
		if (!filter_var($arUpdatePropertyValues[27], FILTER_VALIDATE_EMAIL)){
			$arResult["ERRORS"][] = 'Неверно введён E-mail';
		}

		//pp($arUpdatePropertyValues,true);
		//---BP---
		if($bBizproc)
		{
		...
		if (empty($arResult["ERRORS"]))
		{
			...
			else
			{
				...
				if (!$arParams["ID"] = $oElement->Add($arUpdateValues, $bWorkflowIncluded, true, $arParams["RESIZE_IMAGES"]))
				{
					...
					$arEventFields = array(
						"AUTHOR"           => $arUpdateValues["PROPERTY_VALUES"][26],
						"TEXT"             => $arUpdateValues["PREVIEW_TEXT"],
						"AUTHOR_EMAIL"     => $arUpdateValues["PROPERTY_VALUES"][27],
						"AUTHOR_PHONE"     => $arUpdateValues["PROPERTY_VALUES"][28],
						"EMAIL_TO"         => $arUpdateValues["PROPERTY_VALUES"][27],
						"LINK"             => "http://shantilab.ru/bitrix/admin/iblock_element_edit.php?IBLOCK_ID=10&type=services&ID=".$arParams["ID"]."&lang=ru&find_section_section=0&WF=Y",
					);

					if ($arResult["SELLING_PROP_ELEMENT"]){
						$arEventFields["TARIF"] = "Тариф: ".$arResult["SELLING_PROP_ELEMENT"]["NAME"]." - ".SaleFormatCurrency($arResult["SELLING_PROP_ELEMENT"]["PROPERTIES"]["PRICE"]["VALUE"], "RUB");
					}
